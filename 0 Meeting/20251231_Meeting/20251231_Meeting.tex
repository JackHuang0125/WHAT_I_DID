...
\begin{itemize}
  ...
  \begin{enumerate}[]
    ...
    \item 因此 $\hat p(x_0)\approx 0.5$ 時 local $R^2$ 仍可很高並不矛盾；若在 $\hat p(x_0)\to 0/1$ （黑箱更確定時）， local $R^2$ 下降，表示目前的局部模型$\hat{\hat{g}}$在那些區域不足以逼近$\hat{f}$，需要更強的$\hat{\hat{g}}$
  \end{enumerate}
\end{itemize}
...
\linespread{1.5}
  ...
  \begin{itemize}
    ...
    \begin{enumerate}[]
      ...
      \item 對$x_0$做local surrogate（生成$x_{\ell}=x_0+\epsilon_{\ell}$），計算$y_{\ell}=\hat{f}_c(x_{\ell})$
      ，最後fit模型$\hat{\hat{g}}$
    \end{enumerate}
  \end{itemize}

  \begin{itemize}
    \item \textbf{Try other ML as $\hat f$}
    \begin{enumerate}[]
      \item 使用 Random Forest 與 XGBoost 作為 black-box
    \end{enumerate}
  \end{itemize}
\end{frame}
