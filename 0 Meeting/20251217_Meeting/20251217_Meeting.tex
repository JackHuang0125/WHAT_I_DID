...
\linespread{1.5}
  ...
  \end{itemize}
  \begin{itemize}
    \item 研究  $\hat f(x)$，$x \in \mathcal{N}(X_0)$，觀察模型在local的解釋性、結構
    \begin{enumerate}[]
      \item 建構 \(\hat f\) 的方法（迴歸、機器學習），使用鄰近資料 \((y_k, x_k)\) 建立local
    \end{enumerate}
...
Reference: \href{https://ucinlp.github.io/files/papers/lime-naacl16demo.pdf}{Ribeiro et al.(2016)}
  \begin{itemize}
    \item Given a design point $x_0$
    \item Generate 擾動的 points $x_k = x_0 + \varepsilon_k$, where $\varepsilon_k \sim N(0, \sigma^2 I)$
    \item 代入 black-box model: $y_k = \hat{f}(x_k)$
    \item Fit a simple local model $g(x; x_0)$ on $\{(x_k, y_k)\}$ s.t. $\hat{f}(x) \approx g(x; x_0)$ near $x_0$. 
  \end{itemize}
\end{frame}

\begin{frame}{\textbf{Results II}　{No SMOTE}}

\begin{center}
  \begin{minipage}{0.82\linewidth}
    \centering
    \includegraphics[width=\linewidth]{local0.png}
  \end{minipage}
  \hspace{0.3em}
  \begin{minipage}{0.35\linewidth}
    \centering
    \includegraphics[width=\linewidth]{localpvsR0.png}
  \end{minipage}
\end{center}
...
\begin{center}
  \begin{minipage}{0.82\linewidth}
    \centering
    \includegraphics[width=\linewidth]{local1.png}
  \end{minipage}
  \hspace{0.3em}
  \begin{minipage}{0.35\linewidth}
    \centering
