...
\begin{frame}{Example}
    ...
    \vspace{0.5em}
    \textbf{First estimator:} $E(\hat{\mu}_1)=\mu, \quad\mathrm{Var}(\hat{\mu}_1)=\frac{\sigma^2}{n}, \quad\text{then\quad}\mathrm{MSE}(\hat{\mu}_1)=\frac{\sigma^2}{n}$
    ...
    &\text{Then},\, \mathrm{MSE}(\hat{\mu}_2)=0.64\frac{\sigma^2}{n}+(0.2\mu)^2
    \end{align*}
...
\begin{frame}{Consistency: Does the Estimator Approach the True Value as the Sample Size Increases?}
    ...
    \vspace{0.8em}
    That is, $\qquad \qquad \qquad P(|\hat{\theta}_n-\theta|>\varepsilon)\to 0\quad (n\to\infty),\quad \forall \varepsilon>0$
...
\vspace{0.8em}
$\qquad\qquad\qquad\qquad\qquad\quad\mathrm{Var}(\hat{p})=\frac{1}{n^2}*n*p(1-p)=\frac{p(1-p)}{n}$

\vspace{1.2em}
Therefore
\vspace{-1.4em}
$$E(\hat p)\to p\,\,\,\text{and}\,\,\,\mathrm{Var}(\hat p)=\frac{p(1-p)}{n}\to 0\quad \text{as}\quad n\to\infty$$
So $$\hat{p}\overset{P}{\longrightarrow}p\Rightarrow\hat{p}\,\,\, \text{is consistent}$$ 
%\Rightarrow\hat{p}$ is an unbiased estimator
\end{frame}
